...
def dump_json(obj, wfname, indent=2):
    with codecs.open(wfname, 'w', encoding='utf-8') as wf:
        json.dump(obj, wf, ensure_ascii=False, indent=indent)


def save_results(names, pubs, results, result_file):
    output = {}
    for name in names:
        output[name] = []
        name_pubs = []
    ...
    result_path = join(result_dir, result_file)

    dump_json(output, result_path, indent=4)
    return output, result_path


def get_results(names, pubs, results):
    output = {}
    for name in names:
        output[name] = []
        name_pubs = []
